...
import { Daemons, NetworkBuilder, NetworkInterfaceBuilder } from "start-sdk/lib/mainFn";
import exportInterfaces from "start-sdk/lib/mainFn/exportInterfaces";
import { ExpectedExports } from "start-sdk/lib/types";

export const main: ExpectedExports.main = setupMain(async ({ effects, utils, started }) => {
  /**
   * ======================== Setup ========================
   * 
  ...
  // Create a Network Builder to be used for finding or creating Tor hostnames and LAN ports
  const networkBuilder = NetworkBuilder.of(effects);

  // ------------ Tor ------------

  // Find or generate a random Tor hostname by ID
  const torHostname1 = networkBuilder.getTorHostName("torHostname1");
  ...
  const lanPort1 = networkBuilder.getPort("lanPort1");
  ...
  return Daemons.of({
    ...
    ready: {
      display: { // Optional. If present, the health check will display to the user
        name: "Web Interface",
